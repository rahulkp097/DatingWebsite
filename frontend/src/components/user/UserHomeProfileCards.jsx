...
const UserHomeProfileCards = () => {
  ...
  const isMatchingCity = users.filter(
    (profile) =>
      profile.gender !== userInfo?.gender && profile.city === userInfo?.city
  );

  const isMatchingQualification = users.filter(
    (profile) =>
      profile.gender !== userInfo?.gender &&
      profile.education === userInfo?.education
  );
  ...
  const isMatchingHobbies = users.filter((profile) =>
    profile.hobbies.some(
      (hobby) =>
        profile.gender !== userInfo?.gender && userInfo?.hobbies.includes(hobby)
    )
  );
